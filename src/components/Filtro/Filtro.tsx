...
const Filtro: React.FC<Filtroprop> = ({ tituloFiltro }) => {
  ...
  const getLivro = useCallback(
    (tituloFiltro: string) => {
      let book = livro.filter((item) => item.titulo === tituloFiltro);
        setIdLivro(book[0].id)
        setTitulo(book[0].titulo);
        setAutor(book[0].autor);
        setPublicacao(book[0].publicacao);
        setCadastro(book[0].cadastro);
        setGnero(book[0].genero);
        setDescricao(book[0].descricao);
    },
    [livro]
  );
  ...
  return (
    <>
      {/* <Button onClick={expandirOpen} sx={{ backgroundColor: "rgba(111, 109, 110, 0.4)"}}>Filtrar</Button> */}
      <SearchIcon onClick={expandirOpen} sx={{cursor:"pointer"}}></SearchIcon>
      <Modal
        open={open}
